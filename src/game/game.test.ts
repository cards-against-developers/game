...
test("round timing constants keep reveal spacing at two seconds and winner tension at three seconds", () => {
  assert.equal(AUTO_REVEAL_INTERVAL_MS, 2_000);
  assert.equal(POST_REVEAL_PAUSE_MS, 1_000);
  assert.equal(RESULT_TENSION_MS, 3_000);
  assert.equal(VOTING_DURATION_MS, 30_000);
});

test("expireJudging pauses for one second after revealing a card before highlighting the next one", () => {
  const game = createGame({
    started: true,
    phase: "judging",
    round: 1,
    submissionDeadlineAt: Date.now() - 1,
    revealOrderIds: ["submission-1", "submission-2"],
    revealedSubmissionIds: [],
    highlightedSubmissionId: "submission-1",
    submissions: [
  ...
  assert.equal(game.highlightedSubmissionId, null);
  let deadlineAt = game.submissionDeadlineAt;
  if (deadlineAt === null) {
    assert.fail("Expected post-reveal pause to set a deadline.");
  }
  assert.ok(deadlineAt >= beforeReveal + POST_REVEAL_PAUSE_MS);
  assert.ok(deadlineAt <= Date.now() + POST_REVEAL_PAUSE_MS);

  game.submissionDeadlineAt = Date.now() - 1;
  const beforeHighlight = Date.now();

  expireJudging(game);

  assert.equal(game.phase, "judging");
  assert.deepEqual(game.revealedSubmissionIds, ["submission-1"]);
  assert.equal(game.highlightedSubmissionId, "submission-2");
  deadlineAt = game.submissionDeadlineAt;
  if (deadlineAt === null) {
    assert.fail("Expected next reveal highlight to set a deadline.");
  }
  assert.ok(deadlineAt >= beforeHighlight + AUTO_REVEAL_INTERVAL_MS);
  assert.ok(deadlineAt <= Date.now() + AUTO_REVEAL_INTERVAL_MS);
});

test("openVoting gives players thirty seconds to vote", () => {
  const game = createGame({
    phase: "judging",
    submissions: [
      { id: "submission-1", playerId: "alice", cardIds: ["white-a1"] },
  ...
  const deadlineAt = game.submissionDeadlineAt;
  if (deadlineAt === null) {
    assert.fail("Expected voting to set a deadline.");
  }
  assert.ok(deadlineAt >= before + VOTING_DURATION_MS);
  assert.ok(deadlineAt <= Date.now() + VOTING_DURATION_MS);
});
...
test("submitCardsForPlayer keeps played cards in hand until the next round", () => {
  ...
  assert.deepEqual(game.whiteDiscard, []);
...
test("startNextRound replaces submitted cards after the round finishes", () => {
  ...
  const aliceHand = game.players.find((player) => player.id === "alice")?.hand;
  const bobHand = game.players.find((player) => player.id === "bob")?.hand;

  assert.equal(aliceHand?.length, 10);
  assert.equal(bobHand?.length, 10);
  assert.equal(aliceHand?.includes("white-a1"), false);
  assert.equal(bobHand?.includes("white-b1"), false);
  assert.equal(aliceHand?.[0], "white-next-40");
  assert.equal(aliceHand?.[1], "white-a2");
  assert.equal(bobHand?.[0], "white-next-39");
  assert.equal(bobHand?.[1], "white-b2");
  assert.deepEqual(game.whiteDiscard, ["white-a1", "white-b1"]);
});
...
test("revealing the final submission ends the round but keeps submitted cards until restart", () => {
  ...
  assert.ok(deadlineAt >= Date.now() + 29_900);
  assert.ok(deadlineAt <= Date.now() + 30_100);
  assert.deepEqual(game.players.find((player) => player.id === "alice")?.hand, [
    "white-a1",
    "white-a2"
  ]);
