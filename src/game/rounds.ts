...
import { drawBlackCard, drawWhiteCard, ensurePlayerHand } from "./internal.js";
import { debugLog } from "../utils/debug.js";

export const AUTO_REVEAL_INTERVAL_MS = 2_000;
export const POST_REVEAL_PAUSE_MS = 1_000;
export const RESULT_TENSION_MS = 3_000;
...
export function startNextRound(
  ...
  const submittedCardIds = replaceSubmittedCardsInPlace(game);
  game.submissions = [];
  ...
  game.whiteDiscard.push(...submittedCardIds);
}

function replaceSubmittedCardsInPlace(game: HostGameState): string[] {
  const submittedByPlayer = new Map<string, Set<string>>();
  const discardedCardIds: string[] = [];

  for (const submission of game.submissions) {
    let cardIds = submittedByPlayer.get(submission.playerId);
    if (!cardIds) {
      cardIds = new Set();
      submittedByPlayer.set(submission.playerId, cardIds);
    }

    for (const cardId of submission.cardIds) {
      cardIds.add(cardId);
      discardedCardIds.push(cardId);
    }
  }

  for (const player of game.players) {
    const submittedCardIds = submittedByPlayer.get(player.id);
    if (!submittedCardIds) {
      continue;
    }

    player.hand = player.hand.flatMap((cardId) => {
      if (!submittedCardIds.has(cardId)) {
        return [cardId];
      }

      const replacementCardId = drawWhiteCard(game);
      return replacementCardId ? [replacementCardId] : [];
    });
  }
...
export function openVoting(
  ...
  game.phase = "result";
  game.submissionDeadlineAt = Date.now() + VOTING_DURATION_MS;
  game.winnerSelectionClosed = false;
  game.votingClosed = false;
...
export function revealNextSubmission(game: HostGameState): void {
  ...
  if (game.highlightedSubmissionId === null) {
    const nextHighlightedSubmissionId = revealOrderIds[revealedCount] ?? null;
    if (!nextHighlightedSubmissionId) {
      openVoting(game);
      return;
    }

    const remaining = revealOrderIds.length - revealedCount;
    game.highlightedSubmissionId = nextHighlightedSubmissionId;
    game.submissionDeadlineAt = Date.now() + AUTO_REVEAL_INTERVAL_MS;
  ...
  const nextRevealedCount = game.revealedSubmissionIds.length;
  const remaining = revealOrderIds.length - nextRevealedCount;
  ...
  game.highlightedSubmissionId = null;
  game.submissionDeadlineAt = Date.now() + POST_REVEAL_PAUSE_MS;
  game.announcement = `${remaining} card${remaining === 1 ? "" : "s"} left to reveal.`;
}
...
function closeVoting(game: HostGameState, fallbackAnnouncement: string): void {
  ...
  game.announcement =
    winnerNames.length > 0
      ? `${formatPlayerNames(winnerNames)} tie for the round win.`
      : "Tie round.";
}
