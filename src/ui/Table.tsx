...
function BoardRing(props: {
  ...
  const cards = createMemo(() => buildBoardCards(props.sync()));
  const cardIds = createMemo(() => cards().map((card) => card.id));
        ...
        return (
          ...
          >
            <SubmissionBoardCard
              ...
              resolutionPending={isResolutionPendingCard(props.sync(), card())}
...
function isResolutionPendingCard(
  sync: SyncView,
  card: BoardCardModel
): boolean {
  return Boolean(
    sync.resolutionPending &&
