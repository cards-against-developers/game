...
export function SubmissionBoardCard(props: {
  ...
  const submission = props.sync.submissions.find(
    (entry) => entry.id === props.card.id
  );
  const pickableClass =
  ...
  return (
    <button
      ...
      class={`board-card-node seat-node submission-choice ${props.card.hidden ? "submission-choice-hidden" : ""}${pickableClass}${winnerClass}${freshRevealClass}${clickedRevealClass}${highlightedClass}${votedClass}`}
      style={props.style}
