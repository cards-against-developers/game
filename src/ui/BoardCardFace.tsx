...
export function BoardCardFace(props: {
  ...
  const footerVoteCount = () =>
    boardCardFooterVoteCount(submission(), props.sync.votingClosed ?? false);
  const footerStart = () => footerName();
  const footerEnd = () => footerVoteCount();
...
function boardCardFooterVoteCount(
  ...
  if (votingClosed) {
    return typeof submission.voteCount === "number" && submission.voteCount > 0
