...
export async function expireSingleplayerSubmissionTimer(
  ...
  const expireHostDeadline = async (): Promise<void> => {
    await page.evaluate(() => {
      const singleplayerWindow = window as SingleplayerWindow;
      const host = singleplayerWindow.__singleplayerSeats?.find(
        (seat) => seat.id === "host"
  ...
  await expireHostDeadline();

  const needsNextRevealHighlight = await page.evaluate(() => {
    const singleplayerWindow = window as SingleplayerWindow;
    const host = singleplayerWindow.__singleplayerSeats?.find(
      (seat) => seat.id === "host"
    );
    ...
    const state = host.controller.state.host.state;
    return (
      state.highlightedSubmissionId === null &&
      state.revealedSubmissionIds.length <
        (state.revealOrderIds ?? state.submissions.map((entry) => entry.id))
          .length
    );
  });

  if (needsNextRevealHighlight) {
    await expireHostDeadline();
  }
}
